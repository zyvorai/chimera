...
2. `go test ./...` (including `internal/fixture`'s directory-mode/rescan tests and the `integration/` govmomi-backed end-to-end test)
3. `go vet ./...`
...
See [`docs/TEST_MATRIX.md`](TEST_MATRIX.md) for the full acceptance matrix (authentication, discovery, OVF/export lease, NFC transfer/retry/resume, fault injection, fixture modes including directory-matching and live rescan).
